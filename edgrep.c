...
const int CEOF = 11;  const int CKET = 12;  const int CBACK = 14;  const int CCIRC = 15;  const int STAR = 01; const int READ = 0;  const int WRITE = 1;
...
jmp_buf  savej; char grepbuf[GBSIZE]; void greperror(char);  void grepline(void);
typedef void  (*SIG_TYP)(int); SIG_TYP  oldhup, oldquit;
int main(int argc, char *argv[]) {  char *p1, *p2;  SIG_TYP oldintr;  oldquit = signal(SIGQUIT, SIG_IGN); oldhup = signal(SIGHUP, SIG_IGN);  oldintr = signal(SIGINT, SIG_IGN);
    if (signal(SIGTERM, SIG_IGN) == SIG_DFL) { signal(SIGTERM, quit); }  argv++;
while (argc > 1 && **argv=='-') { switch((*argv)[1]) { case '\0': vflag = 0;  break;
case 'q': signal(SIGQUIT, SIG_DFL);  vflag = 1;  break; case 'o': oflag = 1;  break;
} argv++;  argc--; }
...
setjmp(savej); commands(); quit(0);  return 0; }
    void commands(void) {  unsigned int *a1;  int c, temp;  char lastsep; for (;;) {
 ...
 if (a1==0) {  a1 = zero+1;  if (a1 > dol) { a1--; }  }  addr1 = a1;  if (c == ';') { dot = a1; } }
    ...
    if (addr1==0) { addr1 = addr2; } switch(c) { case EOF:  return;
    ...
    case 'e':  setnoaddr(); if (vflag && fchange) { fchange = 0;  error(Q); } filename(c);  init();
        addr2 = zero;  goto caseread; case 'g':  global(1);  continue;
    case 'p':  case 'P':  newline();  print();  continue;
    case 'Q':  fchange = 0;  case 'q':  setnoaddr();  newline();  quit(0); caseread:
                if ((io = open((const char*)file, 0)) < 0) { lastc = '\n';  error(file); }  setwide();  squeeze(0);
                ninbuf = 0;  c = zero != dol;
                append(getfile, addr2);  exfile();  fchange = c; continue;
            case 'z':  grepline();  continue; case 'a':   add(0);  continue;
            case 'c':   nonzero(); newline(); rdelete(addr1,addr2); append(gettty, addr1-1);
            ...
            case 'u':  nonzero();  newline(); if ((*addr2&~01) != subnewa) { error(Q); }  *addr2 = subolda; dot = addr2; continue; case 'v':   global(0);  continue;
...
unsigned int* address(void) {  int sign;  unsigned int *a, *b;  int opcnt, nextopand;  int c;
    nextopand = -1;  sign = 1;  opcnt = 0;  a = dot; do {
    do c = getchr(); while (c==' ' || c=='\t');
        if ('0'<=c && c<='9') {  peekc = c;  if (!opcnt)  { a = zero; }  a += sign*getnum();
} else switch (c) { case '$':  a = dol; case '.':  if (opcnt) { error(Q); } break; case '\'':
                c = getchr();  if (opcnt || c<'a' || 'z'<c) { error(Q); }  a = zero;
                do { a++; } while (a<=dol && names[c-'a']!=(*a&~01));  break;
            case '?':  sign = -sign; case '/': compile(c);  b = a; for (;;) {
            a += sign;
                    if (a<=zero) { a = dol; }  if (a>dol) { a = zero; }  if (execute(a)) { break; }  if (a==b)  { error(Q); } } break; default:
...
sign = 1;  opcnt++; } while (zero<=a && a<=dol); error(Q);   return 0; }
int advance(char *lp, char *ep) {  char *curlp;  int i;
    for (;;) {
        switch (*ep++) {
            ...
            default: error(Q); } } }
int append(int (*f)(void), unsigned int *a) {  unsigned int *a1, *a2, *rdot;  int nline, tl;  nline = 0;  dot = a; while ((*f)() == 0) {
...
void blkio(int b, char *buf, long (*iofcn)(int, void*, unsigned long)) {
    lseek(tfile, (long)b*BLKSIZE, 0);  if ((*iofcn)(tfile, buf, BLKSIZE) != BLKSIZE) {  error(T);  } } void callunix(void) {
 SIG_TYP savint;  int pid, rpid, retcode;  setnoaddr(); if ((pid = fork()) == 0) {
    signal(SIGHUP, oldhup);  signal(SIGQUIT, oldquit);  execl("/bin/sh", "sh", "-t", 0);  exit(0100);  } savint = signal(SIGINT, SIG_IGN);
...
void compile(int eof) {  int c, cclcnt;  char *ep = expbuf, *lastep, bracket[NBRA], *bracketp = bracket; if ((c = getchr()) == '\n') { peekc = c;  c = eof; }
...
int compsub(void) {  int seof, c;  char *p;  if ((seof = getchr()) == '\n' || seof == ' ') { error(Q); } compile(seof);  p = rhsbuf; for (;;) { c = getchr();
    if (c == '\\') { c = getchr() | 0200; } if (c == '\n') {
...
void dosub(void) { char *lp = linebuf, *sp = genbuf, *rp = rhsbuf;  int c;
  while (lp < loc1) { *sp++ = *lp++; } while ((c = *rp++ & 0377) == 1) {
  ...
  for (a1 = addr1; a1 <= addr2; a1++) {
    if (execute(a1)){  unsigned *ozero;  int m = n;
      ...
      if (m <= 0) {  inglob |= 01;  subnewa = putline();  *a1 &= ~01;
        ...
        nl += zero - ozero;  a1 += nl;  addr2 += nl; } } } if (inglob == 0) { error(Q); } }
